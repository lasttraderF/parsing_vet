...
2. ищет верхнеуровневые разделы в блоке `Sections`;
3. если блок размечен нестандартно, использует запасной поиск по ссылкам внутри страницы;
...
Я усилил логику поиска разделов `Sections`, потому что у сайта этот блок может быть размечен не как обычный заголовок `h2/h3`, а как произвольный текстовый контейнер.

Так как сайт может менять HTML-разметку, селекторы и эвристики могут потребовать лёгкой корректировки в будущем.

В этой среде у меня нет доступа к внешней сети, поэтому код подготовлен аккуратно, но фактический прогон против сайта нужно сделать уже локально.
